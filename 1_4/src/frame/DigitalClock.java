...
public class DigitalClock extends Frame implements ActionListener, Runnable {
	...
	private int pastFontSize;
	private Color pastFontColor;
	private Color pastBgColor;
	private String pastFontName;
	...
	public DigitalClock() {
		...
		try {
			conf = AppConfUseProps.getInstance(AppStyle.CONF_FILE_NAME);//取得先をファイルからPropsに変更
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getActionCommand().equals(AppStyle.Menu.PROPERTY.disp())) {
			tempProp();
			new PropDialog(this);
		}
	}
	...
	private void tempProp(){
		pastFontName = fontName;
		pastFontSize = fontSize;
		pastBgColor = bgColor;
		pastFontColor = fontColor;
	}
	
	private void returnProp(){
		fontName = pastFontName;
		conf.saveProperty(Key.FONT, fontName);
		
		fontSize = pastFontSize;
		conf.saveProperty(Key.FONT_SIZE, Integer.toString(fontSize));		
		
		bgColor = pastBgColor;
		conf.saveProperty(Key.BG_COLOR, AppColor.convertAppColor(bgColor).name());	

		fontColor = pastFontColor;
		conf.saveProperty(Key.FONT_COLOR, AppColor.convertAppColor(fontColor).name());
	...
	class PropDialog extends Dialog implements ActionListener, ItemListener {
		...
		public PropDialog(Frame owner) {
			...
			add(new Label(AppStyle.DialogMenu.FONT.disp(), Label.RIGHT));
			...
			/* fontSizeの選択 */
			add(new Label(AppStyle.DialogMenu.FONT_SIZE.disp(), Label.RIGHT));
			...
			/* font colorの選択 */
			add(new Label(AppStyle.DialogMenu.FONT_COLOR.disp(), Label.RIGHT));
			...
			/* groudColorの選択 */
			add(new Label(AppStyle.DialogMenu.BG_COLOR.disp(), Label.RIGHT));
			...
			Button b2 = new Button(AppStyle.DialogMenu.Cancel.disp());
		...
		public void actionPerformed(ActionEvent e) {
			...
			if(e.getActionCommand().equals(AppStyle.DialogMenu.Cancel.disp())){
				returnProp();
			}
